...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ##############################################################################
    x_h = np.dot(x, Wx)
    prev_h_h = np.dot(prev_h, Wh)
    sum_in = x_h + prev_h_h + b
    next_h = np.tanh(sum_in)
    cache = (x, prev_h, Wx, Wh, sum_in)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    (x, prev_h, Wx, Wh, sum_in) = cache
    dsum = (1 - np.square(np.tanh(sum_in))) * dnext_h
    db = np.sum(dsum, axis=0)
    dx_h = dsum
    dprev_h_h = dsum
    dWh = np.dot(prev_h.T, dprev_h_h)
    dprev_h = np.dot(Wh, dprev_h_h.T).T
    dWx = np.dot(x.T, dx_h)
    dx = np.dot(dx_h, Wx.T)
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def word_embedding_forward(x, W):
    ...
    ##############################################################################
    out = np.array([W[idx] for idx in x])
    cache = (x, W)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0
    for t in range(T):
        prev_h, cell, cache_ = lstm_step_forward(x[:, t, :], prev_h, cell, Wx, Wh, b)
        h[:, t, :] = prev_h
        cache.append(cache_)
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def lstm_backward(dh, cache):
    ...
    db = np.zeros((4*H,))
    for t in reversed(range(T)):
        dx_, dprev_h, dprev_c, dWx_, dWh_, db_ = lstm_step_backward(dh[:, t, :] + dprev_h, dprev_c, cache[t])
        dx[:, t, :] += dx_
        dWx += dWx_
        dWh += dWh_
        db += db_
